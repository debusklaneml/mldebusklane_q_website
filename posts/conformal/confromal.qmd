...
pacman::p_load(tidyverse, explore, tidylog, skimr, plotly, performance, mgcv, patchwork, quantregForest)
```
...
```{r}
lm_plot <- test_preds %>% 
  ggplot(aes(x = lm_predictions, y = y)) + 
  geom_point() +
  theme_minimal()

spline_plot <- test_preds %>% 
  ggplot(aes(x = spline_predictions, y = y)) + 
  geom_point() +
  theme_minimal()
...
conformal_quantile_intervals <- function(train_set, calib_set, new_data, alpha = 0.05) {
  ...
  qrf_model <- quantregForest(x = train_set[, -ncol(train_set)], 
  ...
  q_alpha <- quantile(conformity_scores, probs = 1 - alpha)
...
Need to adjust and test the function above. 
